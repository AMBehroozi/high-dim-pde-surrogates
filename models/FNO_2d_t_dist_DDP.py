...
class FNO2d(nn.Module):
    def __init__(self, modes1, modes2, width, state_size, parameter_size):
        ...
        self.fc0 = nn.Linear(self.parameter_size + self.state_size + 2, self.width) # input channel is : (u(x, y)*n_state par(x, y)*n_prameters, x, y)
...
# FNO without parametrs input (for example: U_in --> U_out)
class FNO2d_direct(nn.Module):
    def __init__(self, modes1, modes2, width, state_size):
        super(FNO2d_direct, self).__init__()
        ...
        self.fc0 = nn.Linear(self.state_size + 2, self.width) # input channel is : (u(x, y)*n_state par(x, y)*n_prameters, x, y)
...
# device = 'cuda'
# u_in = torch.rand(nbatch, s0, s1, state_size).to(device) # [nb, s0, s0, state_size]
...
# model = FNO2d(modes1, modes2,  width, state_size, parameter_size).to(device)
