...
class FNO3d(nn.Module):
    def __init__(self, T_in, T_out, modes_x, modes_y, modes_t, width):
        ...
        self.fc0 = nn.Linear(self.T_in + 4, self.width)
    ...
    def forward(self, x, B):
        x = x.unsqueeze(-2).repeat(1, 1, 1, self.T_out, 1)
        grid = self.get_grid(x.shape, x.device)
        
        B = B.unsqueeze(-1).unsqueeze(-1).repeat(1, 1, 1, self.T_out, 1)
        # torch.Size([2, 64, 64, 40, 10]) torch.Size([2, 64, 64, 40, 3])

        x = torch.cat((x, grid, B), dim=-1)
        x = self.fc0(x)
...
if __name__ == "__main__":
    ...
    model = FNO3d(T_in=5, T_out=10, modes_x=8, modes_y=8, modes_t=8, width=20)
    model = model.to(device)  # Move model to the correct GPU before wrapping with DDP

    nbatch, s0, s1, T_in, T_out = 16, 64, 64, 5, 10
    ...
    u_out = model(u_in, parameters)   # u_in:  [nb, nx, ny,  T_in] parameters: [nb, nx, ny]
